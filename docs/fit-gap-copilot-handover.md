# Fit-Gap Copilot: build handover
...
The Wave Factory proposal promises that country workshops start about 70% prepared. The Fit-Gap Copilot is the service that delivers that. For a scope of BPML process steps it produces a **draft fit-gap register**, where every entry has evidence, a confidence score and a named human who decides.

This build has two jobs, and both matter equally:

1. **Build the Copilot**: an agent that reads the corpus through the two existing engines and proposes fit-gap entries.
2. **Evaluate what has been built**: measure how good the RAG engine, the graph engine and the Copilot actually are, using wave-1 material Solvay has already classified as ground truth.

Non-negotiable principle from the proposal: **AI proposes, humans decide.** Every output has `status = proposed` until a named reviewer accepts it. The Copilot never makes a decision.
...
- The two engines never call each other. The Copilot is a **new third component** that calls both; it does not merge them.
...
- `BPML_ProcessesHierarchyExtended.xlsx` is the authoritative process hierarchy and the Copilot's scope backbone.
...
### E3: Copilot Mode A against the wave-1 ground truth
...
Report scores **with and without** holdout. A large gap between the two means the Copilot was reading labels, not reasoning.
...
FIT_GAP_COPILOT_HANDOVER.md in full before doing anything else. It is the
specification for what you will build: a Fit-Gap Copilot agent and an evaluation
...
2. **Excluded material:** should workshop transcripts (32) be in the RAG index for the Copilot, or only in the Decision Registry later? They raise recall but carry personal data (see the Step 0 data-handling item).
...
5. **The RAG answer model:** is `claude-opus-5` also approved for the Copilot's volume of calls, or should classification use a cheaper model, with opus reserved for synthesis?
