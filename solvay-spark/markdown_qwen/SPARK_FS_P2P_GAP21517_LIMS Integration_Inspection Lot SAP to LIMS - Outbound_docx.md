...
```mermaid
flowchart TD
    A["Inspection Lot Created and Released"] --> B["Send Inspection Lot to LIMS"]
    B --> C["Inspection Lot received"]
    C --> D["LIMS Sample is Processed"]
    D --> E["Equipment integration"]
    E --> F["LIMS Results Recording"]
    F --> G{"Decide if it is one or two step approval"}
    G --> H["Send Results Recording to SAP"]
    G --> I["Sample Authorization QC Decision"]
    H --> J["Results Recordings Received"]
    I --> J
    J --> K{"Decide if it is one or two step approval"}
    K --> L["2 step approval"]
    K --> M["1 step approval"]
    L --> N["Send status update (validation) to SAP"]
    M --> O["Send UD to SAP"]
    N --> P["Status Update Received"]
    O --> Q["UD Received"]
```
...
inspection Lot Created and Re- leased
...
UMS Sampie is Pro- cessed

Equipment integra

UMS Results Recording
...
Send status update (validation) to SAP
...
```mermaid
flowchart TD
    A["CREATE - API InspLot (WC:LIMS*)"] --> B["RECEIVE Metadata from SAP"]
    B -->|"Event Notification"| C["CPI"]
    C -->|"Post data to LIMS"| D["POST data in custom table"]
    A -->|"Event Monitoring & Processing"| E["Event Monitoring & Processing"]
    B -->|"Event Monitoring & Reprocessing with Insights"| F["Event Monitoring & Reprocessing with Insights"]
    F --> G["Event Monitoring & Processing"]
    G --> H["Event Monitoring & Processing"]
    I["CHANGE InspLot (WC:LIMS*)"] --> J["Event Monitoring & Processing"]
    K["CANCEL InspLot (WC:LIMS*)"] --> L["Event Monitoring & Processing"]
```
